...
class CmsisPackDescription:
    ...
    def _parse_devices(self, parent: Element) -> None:
        ...
        for elem in parent:
            if elem.tag == 'memory':
                ...
                newState.arch = elem.attrib.get('Dcore', newState.arch)
    ...
    def _extract_arch(self) -> str:
        archs = []
        for state in self._state_stack:
            elem = state.element
            if elem.tag == 'processor' and 'Dcore' in elem.attrib:
                archs.append(elem.attrib['Dcore'])
            elif state.arch:
                archs.append(state.arch)
        if len(archs) > 0:
            return archs[-1].lower()
        else:
            return None
